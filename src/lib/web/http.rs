...
use crate::web::{ctx, form, PageError, PASSWORD_COOKIE, renderer::Renderer};
...
pub async fn new_clip(
    ...
    if let Some(value) = form.value {
        ...
        Err((Status::BadRequest,
             RawHtml(renderer.render_with_data(ctx::Home::default(),
                                               ("clip", &form.context),
                                               &errors))))
    }
}
...
pub async fn get_clip(
    ...
    fn render_with_status<T>(
        ...
        where T: ctx::PageContext + serde::Serialize + std::fmt::Debug
    {
...
pub async fn get_raw_clip(cookies: &CookieJar<'_>,
                          shortcode: ShortCode,
                          hit_counter: &State<HitCounter>,
                          database: &State<AppDatabase>) -> Result<status::Custom<String>, Status> {
...
pub mod catcher {
    use rocket::{catch, Catcher, catchers};
    use rocket::Request;
...
#[cfg(test)]
pub mod test {
    use rocket::http::hyper::service::service_fn;
    use crate::{data::AppDatabase, service, web::test::init_test_client};
    ...
    fn error_on_missing_clip() {
        ...
        let response = client.get("/clip/nope").dispatch();
    ...
    fn requires_password_when_applicable() {
        ...
        // Block clip when no password is supplied
        let response = client
        ...
        // Retrieve clip when the password is provided
        let response = client
