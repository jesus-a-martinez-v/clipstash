use sqlx::Row;
use crate::data::{DatabasePool, DataError};
use crate::ShortCode;
...
pub async fn new_clip<M: Into<model::NewClip>>(model: M, pool: &DatabasePool) -> Result<model::Clip> {
    let model = model.into();
    let _ = sqlx::query!(
...
pub mod test {
    ...
    fn model_new_clip(shorcode: &str) -> model::NewClip {
        use chrono::Utc;

        model::NewClip {
            clip_id: DbId::new().into(),
            content: format!("content for clip '{}'", shorcode),
            title:None,
            shortcode: shorcode.into(),
